<template>
  <div class="h-screen bg-gray-900 text-white overflow-hidden relative">
    ...
    <div class="relative z-10 flex flex-col h-screen">
      ...
      <div class="h-20 bg-gray-800/90 backdrop-blur-sm border-b border-red-600 flex items-center justify-between px-6">
          ...
          <div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="relative w-48 h-1 bg-gray-700 rounded overflow-hidden">
                <!-- Movie Progress Bar -->
                <div class="absolute top-0 left-0 h-full bg-red-600 rounded animate-pulse"
        ...
        <div class="flex items-center gap-4">
          ...
          <button @click="toggleUserList"
            class="flex items-center gap-2 px-3 py-1 rounded bg-gray-700 hover:bg-gray-600 transition-colors">
            <span>👥</span>
      ...
      <!-- Main Central Area with User List -->
          ...
          <div class="absolute bottom-24 w-full flex justify-center" ref="avatars">
            <div class="flex flex-wrap justify-center gap-6">
              <div v-for="avatar in avatars" :key="avatar.id" class="group relative">
                <div class="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center text-2xl"
                  :class="{ 'opacity-60': avatar.status === 'idle' }">
                  {{ avatar.emoji }}
                </div>
                <div
                  class="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 px-2 py-1 rounded text-sm whitespace-nowrap">
                  {{ avatar.username }}
                </div>
              </div>
            </div>
      ...
      <div class="h-16 bg-gray-800/90 backdrop-blur-sm border-t border-gray-700 flex items-center justify-center gap-4">
        <button v-for="reaction in reactions" :key="reaction.emoji" @click="handleReaction(reaction.emoji)"
          class="group relative px-4 py-2 bg-gray-700 rounded hover:bg-red-600 transition-all hover:scale-110">
          <span class="text-xl">{{ reaction.emoji }}</span>
          <span
            class="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 px-2 py-1 rounded text-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{{
              reaction.label }}</span>
        </button>
      </div>
  ...
  <div class="chat-app">
    <p>Chat Component Loaded</p> <!-- Temporary debug message -->
...
import ChatApp from '@/components/Chat/ChatApp.vue';    // importing ChatApp.vue 
export default {
  components: {
    NavBar,
    ChatApp
